...
Required Browser Proof Plans also declare distinct explicit repo-relative Candidate identity paths. They must be regular hashable files outside canonical `.work` and the retained legacy `.planning` root; do not plan provider APIs or live-byte authenticity claims.
