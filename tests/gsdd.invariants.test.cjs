...
describe('G9 — Generation Manifest Contract', () => {
  const MANIFEST_MODULE = path.join(__dirname, '..', 'bin', 'lib', 'manifest.mjs');
  const GSDD_PATH = path.join(__dirname, '..', 'bin', 'gsdd.mjs');
  ...
  test('gsdd.mjs imports manifest module', () => {
    const gsddContent = fs.readFileSync(GSDD_PATH, 'utf-8');
    assert.ok(gsddContent.includes("from './lib/manifest.mjs'"),
      'gsdd.mjs must import from manifest module. FIX: Add manifest import.');
  });

  test('gsdd.mjs defines FRAMEWORK_VERSION', () => {
    const gsddContent = fs.readFileSync(GSDD_PATH, 'utf-8');
    assert.ok(gsddContent.includes('FRAMEWORK_VERSION'),
      'gsdd.mjs must define FRAMEWORK_VERSION. FIX: Add FRAMEWORK_VERSION constant.');
  });

  test('help text documents --templates flag', () => {
    const gsddContent = fs.readFileSync(GSDD_PATH, 'utf-8');
    assert.ok(gsddContent.includes('--templates'),
      'Help text must document --templates flag. FIX: Add --templates to update command help.');
  });

  test('help text documents --dry flag', () => {
    const gsddContent = fs.readFileSync(GSDD_PATH, 'utf-8');
    assert.ok(gsddContent.includes('--dry'),
      'Help text must document --dry flag. FIX: Add --dry to update command help.');
  });

  test('FRAMEWORK_VERSION is exported for test access', async () => {
    // Use dynamic import to test the export
    const gsddContent = fs.readFileSync(GSDD_PATH, 'utf-8');
    assert.ok(gsddContent.includes('export') && gsddContent.includes('FRAMEWORK_VERSION'),
      'FRAMEWORK_VERSION must be exported. FIX: Add FRAMEWORK_VERSION to export statement.');
  });
});
