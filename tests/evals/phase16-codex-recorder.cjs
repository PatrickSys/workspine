...
function recordCodexTurn(options = {}) {
  ...
  const nativeBase = { event_types: [], item_kinds: [], completion_only_diagnostics: [], thread_id: null, turn_id: null, parse_error: null };
  ...
  if (!problem) {
    try {
      ...
      native = {
        ...
        completion_only_diagnostics: (parsed.completion_only_diagnostics || []).map(({ event_index, item_kind }) => ({ event_index, item_kind })),
