...
function commandPathKey(value) { const normalized = toPosix(value).replace(/\/{2,}/g, '/'); return process.platform === 'win32' ? normalized.toLowerCase() : normalized; } function nativePatchBinary(event) { try { const item = event?.params?.item, actions = Array.isArray(item?.commandActions) ? item.commandActions : [], output = String(item?.aggregatedOutput || '').replaceAll('\r\n', '\n').trimEnd(), match = output.match(/^@echo off\n["']([^"'\r\n]*[\\/]codex\.exe)["'] --codex-run-as-apply-patch %\*$/i); return event?.method === 'item/completed' && item?.type === 'commandExecution' && item.status === 'completed' && item.exitCode === 0 && item.source === 'unifiedExecStartup' && actions.length === 1 && /^\d+$/.test(String(item.processId || '')) && /^\s*Get-Content\s+-Raw\s+\(Get-Command\s+apply_patch\)\.Source\s*$/i.test(String(actions[0]?.command || '')) && match && path.win32.isAbsolute(match[1]) ? commandPathKey(match[1]) : null; } catch { return null; } }
function productChange(event, nativePatchBinaries = new Set()) { const item = event?.params?.item, actions = Array.isArray(item?.commandActions) ? item.commandActions : [], changes = Array.isArray(item?.changes) ? item.changes : Array.isArray(item?.files) ? item.files : [], command = actions.map(action => String(action?.command || '')).join('\n') || String(item?.command || ''), nativePatch = command.match(/^\s*\$([A-Za-z_]\w*)\s*=\s*@'\r?\n([\s\S]*?)\r?\n'@\s*;\s*(?:\$\1\s*=\s*\$\1\.Trim\(\)\s*;\s*)?&\s*['"]([^'"\r\n]*[\\/]codex\.exe)['"]\s+--codex-run-as-apply-patch\s+\$\1\s*$/i), patchOutput = String(item?.aggregatedOutput || '').replaceAll('\r\n', '\n'), patchPaths = nativePatch ? [...nativePatch[2].matchAll(/^\*\*\* (?:Add|Update|Delete) File: (.+)$/gm)].map(([, file]) => toPosix(file)) : [], reportedPaths = [...patchOutput.matchAll(/^[AMD] (.+)$/gm)].map(([, file]) => toPosix(file)), patchAccepted = /^Success\. Updated the following files:\n(?:[AMD] .+\n)+$/.test(patchOutput) && patchPaths.length === reportedPaths.length && patchPaths.every((file, index) => path.posix.normalize(file) === path.posix.normalize(reportedPaths[index])), paths = event?.method === 'item/completed' && ['fileChange', 'file_change'].includes(item?.type) && item.status === 'completed' ? changes.flatMap(change => { const file = typeof change === 'string' ? change : typeof change?.path === 'string' ? change.path : typeof change?.file === 'string' ? change.file : null; return file?.trim() ? [toPosix(file)] : []; }) : event?.method === 'turn/diff/updated' ? [...String(event.params?.diff || '').matchAll(/^diff --git a\/(.+?) b\/(.+)$/gm)].flatMap(([, before, after]) => [toPosix(before), toPosix(after)]) : event?.method === 'item/completed' && item?.type === 'commandExecution' && item.status === 'completed' && item.exitCode === 0 && item.source === 'unifiedExecStartup' && actions.length === 1 && /^\d+$/.test(String(item.processId || '')) && nativePatch && nativePatchBinaries.has(commandPathKey(nativePatch[3])) && patchAccepted ? patchPaths : []; return paths.some(file => { const normalized = path.posix.normalize(file), comparable = process.platform === 'win32' ? normalized.toLowerCase() : normalized, roots = process.platform === 'win32' ? ['agents.md', 'goal.md'] : ['AGENTS.md', 'goal.md']; return normalized && !comparable.startsWith('.work/') && !comparable.startsWith('.agents/') && !comparable.startsWith('inputs/') && !roots.includes(comparable); }); }
