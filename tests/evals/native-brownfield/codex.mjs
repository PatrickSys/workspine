...
function productChange(event) { const item = event?.params?.item, actions = Array.isArray(item?.commandActions) ? item.commandActions : [], changes = Array.isArray(item?.changes) ? item.changes : Array.isArray(item?.files) ? item.files : [], command = actions.map(action => String(action?.command || '')).join('\n') || String(item?.command || ''), nativePatch = command.match(/^\s*\$([A-Za-z_]\w*)\s*=\s*@'\r?\n([\s\S]*?)\r?\n'@\s*;\s*(?:\$\1\s*=\s*\$\1\.Trim\(\)\s*;\s*)?&\s*['"]([^'"\r\n]*[\\/]codex\.exe)['"]\s+--codex-run-as-apply-patch\s+\$\1\s*$/i), paths = ['fileChange', 'file_change'].includes(item?.type) ? changes.map(change => toPosix(change?.path || change?.file || change)) : event?.method === 'turn/diff/updated' ? [...String(event.params?.diff || '').matchAll(/^diff --git a\/(.+?) b\/(.+)$/gm)].flatMap(([, before, after]) => [toPosix(before), toPosix(after)]) : event?.method === 'item/completed' && item?.type === 'commandExecution' && item.status === 'completed' && item.exitCode === 0 && item.source === 'unifiedExecStartup' && item.processId && nativePatch && path.win32.isAbsolute(nativePatch[3]) && /^Success\. Updated the following files:\n(?:[AMD] .+\n)+$/.test(String(item.aggregatedOutput || '').replaceAll('\r\n', '\n')) ? [...nativePatch[2].matchAll(/^\*\*\* (?:Add|Update|Delete) File: (.+)$/gm)].map(([, file]) => toPosix(file)) : []; return paths.some(file => { const normalized = path.posix.normalize(file); return normalized && !normalized.startsWith('.work/') && !normalized.startsWith('.agents/') && !normalized.startsWith('inputs/') && !['AGENTS.md', 'goal.md'].includes(normalized); }); }
...
export function findCheckpointWitness(rows, { consumerRoot, checkpointSha256 }) {
  ...
  rows.forEach((row, eventIndex) => {
    ...
    if (event?.method !== 'item/completed' || item?.type !== 'commandExecution'
      || item.status !== 'completed' || !itemKey || itemKey !== rootKey) return;
