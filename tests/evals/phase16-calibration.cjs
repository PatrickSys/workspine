...
function main() {
  ...
  if (mode === 'check') {
    ...
    catch (error) { const failure = error instanceof CalibrationFailure ? error : new CalibrationFailure('calibration_failure', error.message); return output({ schema_version: 2, contract: CONTRACT, mode, provider_invoked: false, browser_invoked: false, calibration_root: '<CALIBRATION_ROOT>', matrix: { cases: ADMITTED_IDS.length, calibrated_cases: 0, pending_cases: 1, calibrated_bindings: 0, pending_bindings: 3, repetitions: 1, campaign_bindings: campaign.bindings.length }, cases: [], terminal: { status: 'blocked', failure_code: failure.code, message: failure.message, evidence: failure.evidence }, claim_limit: 'No calibration claim: input verification failed.' }, 1); }
    const rows = data.cases.map((item) => ({ id: item.id, admission: item.admission, status: item.admission === 'pending' ? 'pending' : 'ready', digest: digest(item) })); return output({ schema_version: 2, contract: CONTRACT, mode, provider_invoked: false, browser_invoked: false, calibration_root: '<CALIBRATION_ROOT>', matrix: { cases: ADMITTED_IDS.length, calibrated_cases: ADMITTED_IDS.length, pending_cases: 1, calibrated_bindings: 24, pending_bindings: 3, repetitions: 1, campaign_bindings: campaign.bindings.length }, cases: rows, terminal: { status: 'passed', pending_cases: ['docusaurus-browser'], message: 'core and auxiliary calibration inputs are structurally valid and hash-verified' }, claim_limit: 'Offline input validity only; no provider, browser, product, or workflow reliability claim.' });
  }
  const pending = selected.filter((item) => item.admission === 'pending'); if (pending.length) return output({ schema_version: 2, contract: CONTRACT, mode, provider_invoked: false, browser_invoked: false, calibration_root: '<CALIBRATION_ROOT>', matrix: { cases: 1, repetitions: repeat, calibrated_bindings: 0, pending_bindings: 3 }, cases: pending.map((item) => ({ id: item.id, admission: item.admission, status: 'pending', repetitions: 0, digest: digest(item) })), terminal: { status: 'blocked', failure_code: 'calibration_pending', pending_cases: pending.map((item) => item.id), message: 'Docusaurus browser calibration belongs to the Phase 16-06 browser gate' }, claim_limit: 'No calibration claim for pending browser rows.' }, 1);
  ...
  return output({ schema_version: 2, contract: CONTRACT, mode, provider_invoked: false, browser_invoked: false, calibration_root: '<CALIBRATION_ROOT>', matrix: { cases: selected.length, calibrated_cases: selected.length, pending_cases: 1, calibrated_bindings: 24, pending_bindings: 3, repetitions: repeat }, cases: rows, terminal: { status: 'passed', message: `all admitted native red/green/red controls passed ${repeat === 1 ? 'once' : 'twice'}` }, claim_limit: 'Only offline calibration input validity and native oracle controls for the three core and two auxiliary cases; no provider, browser, product, or workflow reliability claim.' });
}
