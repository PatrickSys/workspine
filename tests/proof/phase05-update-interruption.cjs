...
// sha256 of the pinned candidate's package name, so the identity check below never needs
// to spell that name out in source.
...
function catalog() {
  ...
  process.stdout.write(`${JSON.stringify({ phase: '05-05', acceptance: false, classification: 'catalog_only', noProductOrNpmCommand: true, candidate: FIXED_CANDIDATE, package: { identity: `candidate@${PACKAGE_VERSION}`, tarballSha256: TARBALL_SHA256, packageJsonSha256: PACKAGE_JSON_SHA256, readmeSha256: README_SHA256, entrySha256: ENTRY_SHA256, members: PACKAGE_MEMBERS }, cases: CASES, faultKinds: FAULT_KINDS, claimLimits: ['no-signal-handling', 'no-abrupt-process-termination', 'no-power-loss-safety', 'no-multi-file-transactionality', 'no-concurrency-safety'] }, null, 2)}\n`);
}
...
function expectedOwnershipFromInstalled(installed, isolated, invariant) {
  ...
  const probeSource = `import { buildPlanningCliHelperEntries } from ${JSON.stringify(pathToFileURL(renderingPath).href)}; const entries=buildPlanningCliHelperEntries({packageName:${JSON.stringify(installed.packageMeta.name)},packageVersion:${JSON.stringify(PACKAGE_VERSION)},stateDirName:'.work'}); process.stdout.write(JSON.stringify(entries.map(({relativePath,content})=>({relativePath,content}))));`;
