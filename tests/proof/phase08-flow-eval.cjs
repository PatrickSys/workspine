...
function greenfield(root, pack, seed) {
  ...
  const auditRel = '.work/milestone/AUDIT.md'; write(path.join(root, auditRel), '---\nstatus: passed\n---\n\n# Audit\n\nPassed.\n'); const next = transition(candidate.entry, root, ['next', '--json', '--no-update-notice'], candidate.isolated, root); need(next.packet.state === 'audit' || next.packet.state === 'complete', 'lifecycle_contract_failure', 'greenfield next route is not audit/complete', next.packet);
