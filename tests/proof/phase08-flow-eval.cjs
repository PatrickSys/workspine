...
function transition(entry, root, args, isolated, base) { const result = invoke(entry, root, args, isolated, base); assertRun(result.raw, args.slice(0, 2).join(' ')); const packet = parseOutput(result.raw, args.slice(0, 2).join(' ')); const { raw, ...bounded } = result; return { ...bounded, packet }; }
...
function sourceIdentity() { const meta = json(path.join(REPO, 'package.json')); return { package: meta.name, version: meta.version, head: cp.execFileSync('git', ['rev-parse', 'HEAD'], { cwd: REPO, encoding: 'utf8' }).trim(), protected: protectedSnapshot(), git: gitSnapshot(process.env) }; }
function greenfield(root, pack, seed) {
  ...
  const planPacket = transition(candidate.entry, root, ['lifecycle-transition', 'plan', '--plan', planRel, '--authority', 'workflow', '--json', '--no-update-notice'], candidate.isolated, root); rewrite(inputs.plan, 'status: pending', 'status: approved'); const executePacket = transition(candidate.entry, root, ['lifecycle-transition', 'execute', '--plan', planRel, '--authority', 'owner', '--approval-ref', 'phase08-fixture', '--json', '--no-update-notice'], candidate.isolated, root);
  const summaryRel = '.work/phases/01-foundation/01-01-SUMMARY.md'; write(path.join(root, summaryRel), '---\nphase: 01-foundation\nplan: 01\nstatus: complete\n---\n\n# Summary\n\nFixture inputs complete.\n'); const verifyPacket = transition(candidate.entry, root, ['lifecycle-transition', 'verify', '--plan', planRel, '--artifact', summaryRel, '--authority', 'workflow', '--json', '--no-update-notice'], candidate.isolated, root);
  const verificationRel = '.work/phases/01-foundation/01-01-VERIFICATION.md'; write(path.join(root, verificationRel), '---\nphase: 01-foundation\nstatus: passed\n---\n\n# Verification\n\nPassed.\n'); const auditPacket = transition(candidate.entry, root, ['lifecycle-transition', 'audit', '--plan', planRel, '--artifact', verificationRel, '--authority', 'workflow', '--json', '--no-update-notice'], candidate.isolated, root); const next = transition(candidate.entry, root, ['next', '--json', '--no-update-notice'], candidate.isolated, root);
...
function brownfield(root, pack) {
  ...
  const progress = transition(candidate.entry, root, ['lifecycle-preflight', 'progress', '--json', '--no-update-notice'], candidate.isolated, root); const resume = transition(candidate.entry, root, ['lifecycle-preflight', 'resume', '--json', '--no-update-notice'], candidate.isolated, root); const planRel = '.work/brownfield-change/CHANGE.md'; const planned = transition(candidate.entry, root, ['lifecycle-transition', 'plan', '--plan', planRel, '--authority', 'workflow', '--json', '--no-update-notice'], candidate.isolated, root); const executed = transition(candidate.entry, root, ['lifecycle-transition', 'execute', '--plan', planRel, '--approved', 'true', '--approval-ref', 'phase08-brownfield', '--authority', 'workflow', '--json', '--no-update-notice'], candidate.isolated, root); rewrite(path.join(root, planRel), 'Current posture: active', 'Current posture: ready_for_verification'); const verify = transition(candidate.entry, root, ['next', '--json', '--no-update-notice'], candidate.isolated, root); const verified = transition(candidate.entry, root, ['lifecycle-transition', 'verify', '--plan', planRel, '--artifact', '.work/brownfield-change/VERIFICATION.md', '--authority', 'workflow', '--json', '--no-update-notice'], candidate.isolated, root); rewrite(path.join(root, '.work/brownfield-change/VERIFICATION.md'), 'status: complete', 'status: passed'); const audited = transition(candidate.entry, root, ['lifecycle-transition', 'audit', '--plan', planRel, '--artifact', '.work/brownfield-change/VERIFICATION.md', '--authority', 'workflow', '--json', '--no-update-notice'], candidate.isolated, root); rewrite(path.join(root, planRel), 'Current posture: ready_for_verification', 'Current posture: closed'); const closed = transition(candidate.entry, root, ['next', '--json', '--no-update-notice'], candidate.isolated, root); return { scenario_id: 'brownfield', claim: 'packed_brownfield_route_and_close_contract_only', states: [planned.packet.state, executed.packet.state, verify.packet.route_kind, verified.packet.state, audited.packet.state, closed.packet.route_kind], progress: progress.packet, resume: resume.packet, artifacts: [artifact(root, planRel), artifact(root, '.work/brownfield-change/HANDOFF.md'), artifact(root, '.work/brownfield-change/VERIFICATION.md'), artifact(root, '.work/.continue-here.md')], claim_limit: 'No model-driven resume or progress claim.' };
}
function newMilestone(root, pack) { const candidate = installCandidate(root, pack); const init = invoke(candidate.entry, root, ['init', '--auto', '--tools', 'claude', '--no-update-notice'], candidate.isolated, root); assertRun(init.raw, 'new milestone init'); return { scenario_id: 'new-milestone-gate', status: 'not_run', reason: 'missing_history_contract', claim: 'No shipped milestone history was fabricated.' }; }
function broken(root, pack) {
  ...
  const after = fixtureSnapshot(copied); need(after.find((row) => row.path === 'index.js').sha256 !== before.find((row) => row.path === 'index.js').sha256, 'fixture_failure', 'broken fixture mutation did not change hash'); const output = run(process.execPath, ['index.js', '--name', 'Ada'], { cwd: copied, env: process.env }); const failure = output.stdout === 'Hello, Ada!\n' ? null : { code: 'fixture_named_greeting_mismatch', message: 'named greeting oracle rejected broken fixture' }; need(failure, 'fixture_named_greeting_mismatch', 'broken fixture unexpectedly passed', { output: output.stdout }); return { scenario_id: 'broken-fixture', status: 'failed', failure, no_success_terminal: true, output: output.stdout, fixture_hash_before: before, fixture_hash_after: after };
}
function removeRoot(root) { const temp = path.resolve(os.tmpdir()); const absolute = path.resolve(root); need(inside(temp, absolute) && path.basename(absolute).startsWith('gsdd-phase08-flow-eval-'), 'cleanup_failure', 'unsafe cleanup target', { absolute, temp }); fs.rmSync(absolute, { recursive: true, force: false, maxRetries: 2, retryDelay: 100 }); need(!exists(absolute), 'cleanup_failure', 'disposable root survived cleanup'); return { target: absolute, removed: true };
}
function main() {
  ...
  try {
    ...
    const pack = packCandidate(root, identity); emit({ record_type: 'setup', scenario_id: 'setup', candidate: { package: identity.package, version: identity.version, pack_sha256: pack.pack_sha256 }, fixture: { copied_before_pack: true, hashes: fixtureHashes }, network_guard: { proxies_removed: true, registry: 'http://127.0.0.1:9/', attempted: false, blocked: true }, scriptsDisabled: true, git: { mode: 'unused', before: gitBefore }, protected: protectedBefore });
    ...
    const brokenResult = broken(roots['broken-fixture'], pack); emit({ record_type: 'scenario', ...brokenResult });
    need(JSON.stringify(gitBefore) === JSON.stringify(gitSnapshot(process.env)), 'containment_failure', 'host Git snapshot changed'); need(JSON.stringify(protectedBefore) === JSON.stringify(protectedSnapshot()), 'containment_failure', 'protected proof changed');
    cleanup = removeRoot(root); emit({ record_type: 'scenario', scenario_id: 'cleanup', cleanup }); emit({ record_type: 'terminal', status: 'passed', candidate: { package: identity.package, version: identity.version, pack_sha256: pack.pack_sha256 }, cleanup, containment: { git_unchanged: true, protected_unchanged: true, source_not_read_after_pack: true, consumer_git_absent: true }, failure: null });
  } catch (error) {
