...
function updateCacheBytes() {
  return Buffer.from(`${JSON.stringify({ schema: 1, checkedAt: new Date().toISOString(), status: 'available', latestVersion: '0.32.0', error: null })}\n`);
}
...
function losslessSorted(value) { if (Array.isArray(value)) return value.map(losslessSorted); if (value && typeof value === 'object') return Object.fromEntries(Object.keys(value).sort(ordinalCompare).map((key) => [key, losslessSorted(value[key])])); return value; }
function memberLedger(files) {
  need(Array.isArray(files) && files.length > 0, 'provenance_failure', 'npm pack --json returned an empty or missing member set');
  const seen = new Set();
  return files.map((member, index) => {
    need(member && typeof member === 'object' && !Array.isArray(member), 'provenance_failure', `npm pack --json member ${index} was not an object`);
    const memberPath = member.path;
    need(typeof memberPath === 'string' && memberPath && !memberPath.includes('\\') && !memberPath.includes('\0') && !memberPath.startsWith('/') && !/^[A-Za-z]:[\\/]/.test(memberPath), 'provenance_failure', `npm pack --json member ${index} has an invalid path`);
    need(memberPath.split('/').every((part) => part && part !== '.' && part !== '..'), 'provenance_failure', `npm pack --json member ${index} has an invalid path: ${memberPath}`);
    need(Number.isSafeInteger(member.size) && member.size >= 0, 'provenance_failure', `npm pack --json member ${index} has an invalid size`);
    need(Number.isSafeInteger(member.mode) && member.mode >= 0 && member.mode <= 0o777, 'provenance_failure', `npm pack --json member ${index} has an invalid mode`);
    need(!seen.has(memberPath), 'provenance_failure', `npm pack --json member paths contained a duplicate: ${memberPath}`);
    seen.add(memberPath);
...
function canonical() { const head = gitText(['rev-parse', 'HEAD']); const branch = gitText(['branch', '--show-current']); const upstream = gitText(['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{upstream}']); const recovery = gitText(['show-ref', '--verify', '--hash', 'refs/heads/recovery/workspine-relaunch-20260809']); const liveRecovery = gitText(['ls-remote', 'origin', 'refs/heads/recovery/workspine-relaunch-20260809']).split(/\s+/)[0]; const tracked = gitText(['status', '--porcelain=v1', '--untracked-files=no']); const index = run('git', ['diff', '--cached', '--quiet'], { cwd: REPO, env: process.env }); const untracked = gitText(['ls-files', '--others', '--exclude-standard']).split(/\r?\n/).filter(Boolean).sort(); const allowed = ['tests/proof/phase05-concurrency.cjs']; need(branch === 'recovery/workspine-relaunch-20260809' && upstream === 'origin/recovery/workspine-relaunch-20260809' && recovery === head && liveRecovery === head && tracked === '' && index.exitCode === 0 && same(untracked, allowed), 'candidate_drift', 'current recovery branch/upstream/live-ref/index/tracked/untracked predicate drifted', { head, branch, upstream, refs: { recovery, liveRecovery }, tracked, index: index.exitCode, untracked, allowed }); return { head, branch, upstream, refs: { recovery, liveRecovery }, index: 'clean', tracked, untracked, protectedInputs: manifest() }; }
...
function packInstall(root, isolated, candidate) { const source = path.join(root, 'candidate-source'), archive = path.join(root, 'candidate.tar'); const assertNpmAdmission = (stage) => { const rows = isolated.npmAdmissionLedger; need(Object.isFrozen(rows) && rows.length === 2 && rows[0].absolutePath !== rows[1].absolutePath, 'harness_failure', 'npm admission ledger did not retain two distinct config paths', rows); for (const row of rows) { const resolved = fs.realpathSync(row.absolutePath), stat = fs.lstatSync(row.absolutePath), bytes = fs.readFileSync(row.absolutePath); need(resolved === row.absolutePath && inside(root, resolved) && path.relative(root, resolved) === row.relativePath && stat.isFile() && !stat.isSymbolicLink() && bytes.length === row.expectedBytes && sha(bytes) === row.expectedSha256 && row.regularFile === true && row.symbolicLink === false, 'harness_failure', `npm ${row.role} config admission drifted before ${stage}`, { row, resolved, regularFile: stat.isFile(), symbolicLink: stat.isSymbolicLink(), bytes: bytes.length, sha256: sha(bytes) }); } }; fs.mkdirSync(source, { recursive: true }); success(run('git', ['archive', '--format=tar', '--output', archive, candidate.head], { cwd: REPO, env: isolated.env }), 'producer_failure', 'candidate archive'); success(run('tar', ['-xf', archive, '-C', source], { cwd: root, env: isolated.env }), 'producer_failure', 'candidate archive extract'); fs.unlinkSync(archive); assertNpmAdmission('npm pack'); const packed = parse(success(run(process.execPath, [NPM, 'pack', '--ignore-scripts', '--json'], { cwd: source, env: isolated.env }), 'producer_failure', 'npm pack'), 'producer_failure', 'npm pack'); need(Array.isArray(packed) && packed.length === 1 && packed[0] && typeof packed[0].filename === 'string' && path.basename(packed[0].filename) === packed[0].filename, 'provenance_failure', 'npm pack package tuple or filename drifted', packed); const ledger = memberLedger(packed[0].files); const tarball = path.join(source, packed[0].filename); need(inside(source, tarball) && exists(tarball), 'provenance_failure', 'npm pack tarball escaped the candidate source', { tarball }); const tarballSha256 = sha(fs.readFileSync(tarball)); const install = path.join(root, 'install'); fs.mkdirSync(install, { recursive: true }); write(path.join(install, 'package.json'), '{"name":"phase05-lifecycle-routes","private":true}\n'); assertNpmAdmission('local tarball install'); success(run(process.execPath, [NPM, 'install', '--ignore-scripts', '--no-audit', '--no-fund', '--offline', tarball], { cwd: install, env: isolated.env }), 'producer_failure', 'local tarball install'); const packageName = JSON.parse(fs.readFileSync(path.join(source, 'package.json'), 'utf8')).name; const packageRoot = path.join(install, 'node_modules', packageName); const entry = fs.realpathSync(path.join(packageRoot, 'bin', 'gsdd.mjs')); const entrySha256 = sha(fs.readFileSync(entry)); need(inside(install, entry), 'provenance_failure', 'installed entry escaped install root', { entry }); return { source, install, packageRoot, entry, tarball: { sha256: tarballSha256, members: ledger.length, memberLedger: ledger, filename: packed[0].filename }, entrySha256, npmAdmission: { ledger: isolated.npmAdmissionLedger, assertions: ['pre_npm_pack', 'pre_local_tarball_install'] } }; }
...
function nextSemantic(root, baseline, mode) { const label = mode === 'cold' ? 'cold next' : 'warm next'; need(mode === 'cold' || mode === 'warm', 'harness_failure', 'unknown next semantic mode'); const afterNext = lstatTree(root); const changes = delta(baseline, afterNext); need(same(changes, []), 'product_mismatch', label + ' changed the saved fixture snapshot', changes); const cachePaths = (snapshot) => snapshot.entries.filter(({ path: entryPath }) => entryPath === '.work/.local' || entryPath.startsWith('.work/.local/')); const beforeCache = cachePaths(baseline); const afterCache = cachePaths(afterNext); need(same(afterCache, beforeCache), 'product_mismatch', label + ' created or changed the update-awareness cache', { before: beforeCache, after: afterCache }); Object.defineProperty(afterNext, 'cacheReceipt', { value: { delta: [], snapshot: { entries: afterCache }, entries: afterCache.map(({ path: entryPath }) => entryPath) }, enumerable: false }); return afterNext; } function noWrite(before, root, label) { need(same(before, lstatTree(root)), 'product_mismatch', `${label} changed a read/refusal fixture`, { delta: delta(before, lstatTree(root)) }); }
function seedUpdateCache(root) { const cache = path.join(root, '.work', '.local', 'update-awareness.json'); write(cache, updateCacheBytes()); return cache; }
...
function catalog() { const before = manifest(); need(CASE_IDS.length === 7 && new Set(CASE_IDS).size === 7, 'catalog_failure', 'closed seven-case table drifted'); need(same(before, manifest()), 'catalog_failure', 'catalog changed protected inputs'); process.stdout.write(`${JSON.stringify({ phase: '05-07', acceptance: false, classification: 'catalog_only', candidateBinding: { derivedAtRunStart: true, tarballSha256: 'derived_from_run', members: 'derived_from_run', installedEntryRequired: true, requiresCleanRecoveryParity: true }, cases: CASE_IDS.map((id) => ({ id })), nonClaims: { ...HUMAN_GATE, blind_reader: 'not_proven', native_agent: 'not_proven', interactive: 'not_proven', browser: 'not_proven' }, noProductNpmFixtureActivity: true }, null, 2)}\n`); }
function development() {
  ...
  try {
    ...
    const installed = packInstall(root, isolated, before);
    const cases = [
    ...
    return { phase: '05-07', acceptance: false, classification: 'p05-07a_lifecycle_route_acceptance_development_harness_pass', lifecycleAcceptance: 'P05-07A', exercisedLevel: 'packed_local_installed_entry_seeded_fixture', candidate: before, after, installed: { tarball: installed.tarball, entry: installed.entry, entrySha256: installed.entrySha256, npmAdmission: installed.npmAdmission }, cases, cleanup: cleanupReceipt, failureGuard, nonClaims: { ...HUMAN_GATE, blind_reader: 'not_proven', native_agent: 'not_proven', interactive: 'not_proven', browser: 'not_proven', release_acceptance: 'not_proven', commit_push_publication: 'not_performed' } };
  } catch (error) {
