...
function casePacked(installed, root, isolated) { initGit(root, isolated); const before = lstatTree(root); success(invoke(installed, root, ['init', '--auto', '--tools', 'agents'], isolated), 'product_mismatch', 'installed init'); const quick = path.join(root, '.agents', 'skills', 'gsdd-quick', 'SKILL.md'); const routes = ['gsdd-quick', 'gsdd-plan', 'gsdd-execute', 'gsdd-verify', 'gsdd-resume'].map((name) => { const file = path.join(root, '.agents', 'skills', name, 'SKILL.md'); const value = fs.readFileSync(file, 'utf8'); need(value.trim().length > 0 && value.includes(name), 'product_mismatch', `generated ${name} route is absent or empty`); return { route: name, sha256: sha(Buffer.from(value)) }; }); const readme = fs.readFileSync(path.join(installed.packageRoot, 'README.md'), 'utf8'); need(readme.includes('gsdd-quick') && readme.includes('gsdd-plan') && readme.includes('gsdd-execute') && readme.includes('gsdd-verify'), 'product_mismatch', 'packed README route surface is incomplete'); need(delta(before, lstatTree(root)).length > 0 && exists(quick), 'product_mismatch', 'init did not produce expected generated surfaces'); return { id: CASE_IDS[0], evidence: 'packed_static_generated_route_contract', routes, readmeSha256: sha(Buffer.from(readme)), claimLimit: 'not_blind_reader_or_native_agent_evidence' }; }
...
function caseNewProject(installed, root, isolated) {
  ...
  const generatedPaths = [".agents/skills/gsdd-new-project/SKILL.md", ".agents/skills/gsdd-new-milestone/SKILL.md", ".agents/skills/gsdd-execute/SKILL.md", ".agents/skills/gsdd-verify/SKILL.md", ".work/bin/gsdd.mjs"];
  ...
  const missingBrief = "Auto mode requires a project brief. Provide one via " + tick + "npx -y gsdd-cli init --auto --tools <runtime> --brief <path>" + tick + " or place it at " + tick + ".work/PROJECT_BRIEF.md" + tick + ".";
  ...
  need(staticText.change.includes("one active medium-scope change only") && staticText.change.includes(".work/brownfield-change/CHANGE.md") && staticText.change.includes("one shared goal") && staticText.change.includes("closeout path") && staticText.change.includes("disjoint write ownership") && staticText.change.includes("/gsdd-new-project") && staticText.change.includes("/gsdd-new-milestone"), "harness_failure", "brownfield CHANGE markers drifted");
  ...
  const executeSkill = fs.readFileSync(path.join(root, ".agents", "skills", "gsdd-execute", "SKILL.md"), "utf8");
  const verifySkill = fs.readFileSync(path.join(root, ".agents", "skills", "gsdd-verify", "SKILL.md"), "utf8");
