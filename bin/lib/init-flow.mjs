...
/**
 * Read-only parity check for the repair command health would advertise for a
 * missing native runtime target. This deliberately follows the same init
 * ownership/template/config preflight without applying recovery or writing
 * generated bytes.
 */
export function preflightLocalInitRepair(ctx, runtime) {
  try {
    const state = resolveStateDir(ctx.cwd);
    const gate = stateAuthorityGate(state);
    if (!gate.allowed) throw new Error(gate.message);
    const initCtx = contextAtWorkspaceRoot(ctx, ctx.cwd);
    preflightInitState(initCtx, { isAuto: false });
    const targets = getLocalAdapterTargets(initCtx.adapters, initCtx.workflows, [runtime]);
    planAdapterGeneration({
      cwd: initCtx.cwd,
      planningDir: initCtx.planningDir,
      targets,
...
/**
 * Read-only structural preflight for repository update repair guidance. Missing
 * native targets are allowed here because health orders their init repair
 * before plain update; every other ownership/provenance/template blocker is
 * checked against the same update planner used by the mutator.
 */
export function preflightLocalUpdateRepair(ctx) {
  try {
    const state = resolveStateDir(ctx.cwd);
    const gate = stateAuthorityGate(state);
    if (!gate.allowed) throw new Error(gate.message);
    const updateCtx = contextAtWorkspaceRoot(ctx, ctx.cwd);
    const { planningDir, stateDirName } = updateCtx;
    ...
    const platforms = manifestPlatforms.length > 0 ? manifestPlatforms : detectPlatforms(updateCtx.adapters);
    const adaptersToUpdate = resolveAdapters(updateCtx.adapters, platforms);
    const writerPlatforms = [...new Set([...platforms, ...adaptersToUpdate.map((adapter) => adapter.name)])];
    const targets = getLocalAdapterTargets(updateCtx.adapters, updateCtx.workflows, writerPlatforms);

    if (existsSync(planningDir)) {
      validateTemplateOwnership(planningDir);
      planTemplateRefresh({ ...updateCtx, isDry: true });
    }
    planAdapterGeneration({
      cwd: updateCtx.cwd,
      planningDir,
      ...
      // A preceding init repair may be the step that restores these. Keep the
      // structural/provenance checks exact without rejecting that valid chain.
