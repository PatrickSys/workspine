...
import { existsSync, mkdirSync, readFileSync, writeFileSync, readdirSync, cpSync, realpathSync, unlinkSync } from 'fs';
...
function refreshTemplates(isDry) {
  const existingManifest = readManifest(PLANNING_DIR);
  const globalTemplatesDir = join(DISTILLED_DIR, 'templates');
  const localTemplatesDir = join(PLANNING_DIR, 'templates');
  const agentsDir = join(__dirname, '..', 'agents');
  ...
  for (const cat of categories) {
    refreshCategory(cat, existingManifest, isDry);
  }
...
function refreshCategory({ name, src, dest, manifestKey }, existingManifest, isDry) {
  ...
  const sourceFiles = readdirSync(src).filter(f => f.endsWith('.md'));
  const installedFiles = existsSync(dest) ? readdirSync(dest).filter(f => f.endsWith('.md')) : [];
  ...
  for (const file of sourceFiles) {
    ...
    if (existsSync(destPath)) {
      ...
      // Check if user modified (installed hash differs from manifest hash)
  ...
  // Remove orphans (installed but no longer in source)
...
function refreshRootTemplates(globalTemplatesDir, localTemplatesDir, existingManifest, isDry) {
  ...
  const sourceFiles = readdirSync(globalTemplatesDir).filter(f => f.endsWith('.md'));
...
function refreshRoles(agentsDir, localRolesDir, existingManifest, isDry) {
  ...
  const sourceFiles = readdirSync(agentsDir).filter(
    f => f.endsWith('.md') && f !== 'README.md' && !f.startsWith('_')
  );
  const installedFiles = existsSync(localRolesDir) ? readdirSync(localRolesDir).filter(f => f.endsWith('.md')) : [];
