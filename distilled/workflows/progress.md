...
All `node .agents/bin/gsdd.mjs ...` helper references below assume the current working directory is the repo root. If the runtime launched from a subdirectory, change to the repo root before acting on them.
</repo_root_helper_contract>
...
- Do not call `node .agents/bin/gsdd.mjs phase-status` here.
- If you recommend a next step that crosses a lifecycle boundary, the downstream mutating workflow must rerun its own `node .agents/bin/gsdd.mjs lifecycle-preflight ...` gate before acting.
</lifecycle_boundary>
