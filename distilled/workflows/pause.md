...
<next_action>
[The specific first thing to do when resuming — include the exact command or file/step reference verbatim, quoted, so a fresh session can act without interpretation]
</next_action>
...
- How to resume: run the `/work-resume` workflow in the next session
</confirm>
...
**Next step (next session):** `/work-resume` — restore context and continue where you left off

Also available:
- `/work-progress` — check project status without restoring checkpoint context
