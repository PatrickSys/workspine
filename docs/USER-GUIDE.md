...
4. **Implement.** Run `work-execute` in chat after approval. If the agent cannot confirm that approval was recorded, ask it to show the approval record before proceeding. The agent follows the approved scope and records what changed and which checks ran. A material unresolved owner choice returns to you.
